...
@section('content')
    <section class="tf-section2 flat-blog-grid flat-property style3 tf-section">
        <div class="container">
            <div class="row flex">
                <div class="col-lg-12">
                    <div class="post">
                        <div class="category-filter flex  justify-space">
                            <div class="box-1 ">
                                <div class="heading-listing fs-30 lh-45 fw-7">المدونة</div>
                            </div>
                        ...
                        <div class="flat-blog">
                            <div class="box-heading ">
                                <div class="image relative ">
                                    @if ($firstPost->image)
                                        <img src="{{ url('/storage/app/public/' . $firstPost->image) }}" alt="images">
                                    @else
                                        <img src="https://via.placeholder.com/400x250?text=No+Image" alt="images">
                                    @endif

                                    <div class="sub-box flex align-center fs-13 fw-6">
                                        <div class="title-1">
                                            {{ \Carbon\Carbon::parse($firstPost->created_at)->format('F') }}</div><a
                                            class="title-2 text-color-3">{{ $firstPost->categoryBlog->name }}</a>
                                    </div>
                                </div>
                                <div class="contents center">
                                    <div class="title-heading fs-30 fw-7 lh-45">{{ $firstPost->title }}</div>
                                    <h4 class="fw-4">{{ $firstPost->meta_description }}</h4>
                                </div>
                            </div>
                                ...
                                @forelse($posts as $post)
                                    <div class="box hover-img wg-box">
                                        <div class="images img-style relative ">
                                            ...
                                            @if ($post->image)
                                                <img src="{{ url('/storage/app/public/' . $post->image) }}" alt="images">
                                            @else
                                                <img src="https://via.placeholder.com/400x250?text=No+Image" alt="images">
                                            @endif
                                            ...
                                            <div class="sub-box flex align-center fs-13 fw-6">
                                                <div class="title-1">
                                                    {{ \Carbon\Carbon::parse($post->created_at)->format('F') }}</div><a
                                                    class="title-2 text-color-3">{{ $post->categoryBlog->name }}</a>
                                            </div>
                                        </div>
